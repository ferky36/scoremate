  ...
  function buildTopRankingCard(standings, matches, avgMargin){
    ...
    if (!top.length){
      ...
      top.forEach((p, idx)=>{
        ...
        avWrap.className = 'relative flex-shrink-0';

        const av = document.createElement('div');
        av.className = 'insight-avatar relative overflow-hidden'; // Ensure rounded/overflow for image
        ...
        if (idx < 3) {
           ...
           // Badge style: top-left corner
           badge.className = 'absolute -top-1 -left-1 w-5 h-5 flex items-center justify-center rounded-full shadow-sm z-10 ring-1 ring-white dark:ring-slate-900';
           
           if (idx===0) badge.classList.add('bg-yellow-400'); // Gold
           else if (idx===1) badge.classList.add('bg-slate-300'); // Silver
           else badge.classList.add('bg-amber-600'); // Bronze
           
           // Star Icon
           badge.innerHTML = `<svg class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>`;
