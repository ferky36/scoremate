...
// Imported modules from refactored code
import { formatTime, showToast } from './tennis/tennis-utils.js';
import { overlayTemplate, styleCss as templateStyleCss, renderServeBallSvg } from './tennis/tennis-ui-template.js';
import { ServerRotation } from './tennis/server-rotation.js';
import { WakeLockManager } from './tennis/wake-lock.js';
import { RallySettings } from './tennis/rally-settings.js';
import { ModalManager } from './tennis/modal-manager.js';
...
(function(){
  ...
  // REFACTORED: Initialize ModalManager
  const modalManager = new ModalManager(__tsT);

  // Build overlay HTML (copied and adapted from enhancement/tennis-score.html body)
  // REFACTORED: Use imported template and style
  // const overlayHtml = `...` replaced by overlayTemplate()
  const overlayHtml = overlayTemplate();

  // const styleCss = `...` replaced by templateStyleCss
  const styleCss = templateStyleCss;
  // REFACTORED: imported from tennis-ui-template.js
  const MAX_SERVE_BADGE_BALLS = 2; // Keep local const or import if needed
  // renderServeBallSvg is imported at top of file
  ...
  // REFACTORED: Initialize class instances for refactored modules
  let serverRotation = new ServerRotation(1); // Start with player 1
  let wakeLockManager = new WakeLockManager();
  let rallySettings = null; // Will be initialized when court is known
    ...
    try{
      if (window.__rallyFinishCheckbox){
        window.__rallyFinishCheckbox.addEventListener('change', (ev)=>{
          ...
          // REFACTORED: Use module to save
          if (rallySettings) rallySettings.saveFinishAt21(state.rallyFinishAt21);
  ...
  // REFACTORED: rotateServer now uses ServerRotation class
  function rotateServer(){
    serverRotation.rotate();
    state.currentPlayerServer = serverRotation.getCurrentServer();
    resetActiveServerBadge();
  }
  // Old inline implementation (commented out):
  // function rotateServer(){
  ...
  // REFACTORED: showToast now imported from tennis-utils.js
  const tsShowToast = showToast; // Alias for backward compatibility
...
function confirmAction(){
  ...
  function transitionToLiveModeAfterForceReset(){
    state.pendingClearScore = false; try{ wakeLockManager.release(); }catch{};
  ...
  function performMatchReset(){
    if (state.timerInterval){ clearInterval(state.timerInterval); state.timerInterval=null; }
    state.isMatchRunning=false; try{ wakeLockManager.release(); }catch{}; resetMatch(true,true);
    setStartButtonLabel();
      ...
      try{
        ...
        if (state.rallyFinishAt21 && !state.finishAt21Prompted){
          if (totalPointsNow >= 21){
            ...
            showConfirmationModal('finish21');
            // Inline logic removed/commented
            /*
  ...
  function handleGameWin(winningTeam){
    ...
    // Calculate next server for display
    ...
    const nextServerName = playerDetails[nextServerId]?.name || '';
    
    // REFACTORED: Use ModalManager
    if (modalManager) {
      modalManager.showGameWonModal(winnerName, nextServerName);
    }
    // Inline logic removed/commented
    /*
  ...
  async function finishMatch(fromTimer=false, confirmed=false){
    ...
    state.isMatchFinished=true; state.isMatchRunning=false; state.gameWinPending=false; try{ wakeLockManager.release(); }catch{};
    ...
    const winnerNames = winnerTeam ? formatTeamNames(winnerTeam) : '';
    // REFACTORED: Use ModalManager
    if (modalManager) {
      if (typeof updateNextMatchInfo === 'function') updateNextMatchInfo(tsCtx.court, tsCtx.round);
      modalManager.showMatchResultsModal({
        gamesT1: state.gamesT1,
        gamesT2: state.gamesT2,
        winner: matchWinner,
  ...
  function resetMatch(hideModals=true, fullReset=false){
    ...
    updateDisplay();
    try{ if (!state.isMatchRunning) wakeLockManager.release(); }catch{}
  }
  ...
  window.openScoreModal = function(courtIdx, roundIdx){
    if (state.isMatchRunning){ showToast(__tsT('tennis.error.matchRunning','Pertandingan sedang berlangsung!'), 'error'); return; }
    tsCtx.court = courtIdx; tsCtx.round = roundIdx;

    // REFACTORED: Initialize RallySettings for this court
    rallySettings = new RallySettings(courtIdx);

    ensureOverlay(); resetMatch(true,true);
