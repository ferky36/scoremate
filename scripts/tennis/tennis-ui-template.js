  ...
  <div id="tsOverlay" class="fixed inset-0 z-50 hidden items-center justify-center p-3 md:p-6 transition-colors duration-300">
    ...
    <div class="relative w-full max-w-xl bg-white dark:bg-[#0f172a] shadow-2xl rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[98vh] text-gray-900 dark:text-white transition-colors duration-300">
      ...
      <div class="flex items-center justify-between p-4 border-b border-gray-100 dark:border-white/5">
        <div>
          <h1 id="tsTitle" class="text-xl md:text-2xl font-bold tracking-tight">Lapangan 1 - Match 1</h1>
          <p id="tsSchedule" class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 font-medium leading-none"></p>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4">
        <!-- Settings Bar -->
        <div class="p-3 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
          <div class="flex flex-col gap-3">
            <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <label for="mode-selector" class="text-sm font-bold text-gray-600 dark:text-gray-300 whitespace-nowrap">${__tsT('tennis.modeLabel','Pilih Metode Skor:')}</label>
              <select id="mode-selector" class="w-full sm:flex-1 py-2 px-3 bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none text-gray-900 dark:text-gray-100">
            ...
            <div id="rally-finish-wrap" class="hidden">
              <label class="inline-flex items-center gap-2.5 cursor-pointer">
                <input id="rally-finish-21" type="checkbox" class="h-4 w-4 rounded border-gray-300 dark:border-white/20 bg-white/10 text-indigo-600 focus:ring-indigo-500" />
                <span id="rally-finish-label" class="text-sm text-gray-600 dark:text-gray-300 font-medium">${__tsT('tennis.rally.finishAt21Label','Finish at 21 points')}</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div id="timer-display" class="py-4 px-4 bg-amber-50 dark:bg-[#1e293b] text-[#d97706] dark:text-[#f59e0b] text-3xl font-mono font-bold text-center rounded-2xl border-2 border-amber-500/40 dark:border-[#f59e0b]/40 shadow-sm flex items-center justify-center">11:00</div>
          <button id="start-match-btn" class="h-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white text-base font-bold rounded-2xl shadow-lg shadow-indigo-500/30 transition active:scale-[0.98]">
            ${__tsT('tennis.start','Mulai Pertandingan')}
          </button>
        </div>

        <div id="status-message" class="text-center text-xs font-semibold text-indigo-600 dark:text-indigo-400 empty:hidden py-1 px-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">${__tsT('tennis.status.chooseMode','Pilih mode skor dan tekan Mulai Pertandingan.')}</div>

        <!-- Sets Tracker Slab (Moved to Top) -->
        <div id="game-score-display" class="flex items-center justify-between px-5 py-3 bg-indigo-600 dark:bg-indigo-500 text-white rounded-2xl shadow-lg shadow-indigo-500/20">
          <span id="set-score-label" class="text-[10px] font-black tracking-widest uppercase opacity-80">${__tsT('tennis.set.gamesLabel','Sets')}</span>
          <div class="flex items-center gap-6 font-black">
            <div class="flex items-baseline gap-2">
              <span class="text-[10px] font-medium opacity-70">TIM A</span>
              <span id="games-t1" class="text-2xl">0</span>
            </div>
            <div class="h-4 w-[2px] bg-white/20 rounded-full"></div>
            <div class="flex items-baseline gap-2">
              <span class="text-[10px] font-medium opacity-70">TIM B</span>
              <span id="games-t2" class="text-2xl">0</span>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <!-- Team A -->
          <div class="p-3 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-500/10 dark:border-blue-500/30 rounded-[2rem] flex flex-col items-center transition-all">
            <h2 class="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.2em] mb-3">${__tsT('tennis.teamA','TIM A')}</h2>
            
            <div class="flex justify-center gap-10 mb-4 w-full py-2 bg-white/40 dark:bg-blue-800/20 rounded-xl border border-blue-100/30 dark:border-transparent">
              <div class="player-col flex flex-col items-center gap-1.5 flex-1 max-w-[70px]">
                <div class="relative">
                  <img id="player-1-avatar" src="icons/default-avatar.png" class="w-12 h-12 rounded-full border-2 border-white dark:border-gray-800 shadow-md object-cover bg-gray-100 dark:bg-gray-700" />
                  <span class="serve-badge absolute bottom-0 right-0" data-serve-player="1"></span>
                </div>
                <p id="player-1-name" class="text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400 truncate w-full text-center">P1</p>
              </div>
              <div class="player-col flex flex-col items-center gap-1.5 flex-1 max-w-[70px]">
                <div class="relative">
                  <img id="player-2-avatar" src="icons/default-avatar.png" class="w-12 h-12 rounded-full border-2 border-white dark:border-gray-800 shadow-md object-cover bg-gray-100 dark:bg-gray-700" />
                  <span class="serve-badge absolute bottom-0 right-0" data-serve-player="2"></span>
                </div>
                <p id="player-2-name" class="text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400 truncate w-full text-center">P2</p>
              </div>
            </div>

            <div class="text-6xl sm:text-7xl font-black text-blue-600 dark:text-blue-500 leading-none tabular-nums mb-6 drop-shadow-sm" id="score-t1">0</div>
            
            <div class="flex gap-3 w-full px-1">
              <button class="score-btn flex-1 py-3 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/20 text-gray-500 dark:text-white font-black rounded-2xl shadow-sm" data-team="1" data-delta="-1">−</button>
              <button class="score-btn flex-1 py-3 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl shadow-lg shadow-blue-500/30" data-team="1" data-delta="1">+</button>
            </div>
          </div>

          <!-- Team B -->
          <div class="p-3 bg-red-50/50 dark:bg-red-900/10 border border-red-500/10 dark:border-red-500/30 rounded-[2rem] flex flex-col items-center transition-all">
            <h2 class="text-[10px] font-black text-red-600 dark:text-red-400 uppercase tracking-[0.2em] mb-3">${__tsT('tennis.teamB','TIM B')}</h2>
            
            <div class="flex justify-center gap-10 mb-4 w-full py-2 bg-white/40 dark:bg-red-800/20 rounded-xl border border-red-100/30 dark:border-transparent">
              <div class="player-col flex flex-col items-center gap-1.5 flex-1 max-w-[70px]">
                <div class="relative">
                  <img id="player-3-avatar" src="icons/default-avatar.png" class="w-12 h-12 rounded-full border-2 border-white dark:border-gray-800 shadow-md object-cover bg-gray-100 dark:bg-gray-700" />
                  <span class="serve-badge absolute bottom-0 right-0" data-serve-player="3"></span>
                </div>
                <p id="player-3-name" class="text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400 truncate w-full text-center">P3</p>
              </div>
              <div class="player-col flex flex-col items-center gap-1.5 flex-1 max-w-[70px]">
                <div class="relative">
                  <img id="player-4-avatar" src="icons/default-avatar.png" class="w-12 h-12 rounded-full border-2 border-white dark:border-gray-800 shadow-md object-cover bg-gray-100 dark:bg-gray-700" />
                  <span class="serve-badge absolute bottom-0 right-0" data-serve-player="4"></span>
                </div>
                <p id="player-4-name" class="text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400 truncate w-full text-center">P4</p>
              </div>
            </div>

            <div class="text-6xl sm:text-7xl font-black text-red-600 dark:text-red-500 leading-none tabular-nums mb-6 drop-shadow-sm" id="score-t2">0</div>
            
            <div class="flex gap-3 w-full px-1">
              <button class="score-btn flex-1 py-3 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/20 text-gray-500 dark:text-white font-black rounded-2xl shadow-sm" data-team="2" data-delta="-1">−</button>
              <button class="score-btn flex-1 py-3 bg-red-600 hover:bg-red-500 text-white font-black rounded-2xl shadow-lg shadow-red-500/30" data-team="2" data-delta="1">+</button>
            </div>
          </div>
        </div>
      ...
      <!-- Footer Actions -->
      <div class="p-4 border-t border-gray-100 dark:border-white/5 bg-white dark:bg-[#0f172a] flex flex-col gap-3 relative z-10">
        <button id="finish-match-btn" class="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-black rounded-2xl shadow-xl shadow-emerald-500/30 transition active:scale-[0.98] uppercase tracking-wider">${__tsT('tennis.finishMatch','Selesai & Simpan')}</button>
        <button id="force-reset-btn" class="hidden w-full py-4 bg-rose-600 hover:bg-rose-500 text-white text-sm font-black rounded-2xl shadow-xl shadow-rose-500/30 transition active:scale-[0.98] uppercase tracking-wider">Reset</button>
      </div>
    </div>
    ...
    <div id="game-won-modal" class="hidden fixed inset-0 bg-slate-900/40 dark:bg-[#0f172a]/90 backdrop-blur-xl z-[60] items-center justify-center p-4">
      <div class="bg-white dark:bg-[#1e293b] rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl text-center border border-gray-200 dark:border-white/10 transition-all text-gray-900 dark:text-white">
        ...
        <button id="start-new-game-btn" class="mt-8 w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl shadow-xl shadow-emerald-500/30 transition active:scale-[0.98] uppercase tracking-wider">${__tsT('tennis.startNextGame','Lanjut Game Baru')}</button>
      </div>
    </div>
    ...
    <div id="match-results-modal" class="hidden fixed inset-0 bg-slate-900/40 dark:bg-[#0f172a]/95 backdrop-blur-2xl z-[60] items-center justify-center p-4">
      <div class="bg-white dark:bg-[#1e293b] rounded-[3rem] p-8 md:p-10 max-w-md w-full shadow-2xl text-center border border-gray-200 dark:border-white/10 relative text-gray-900 dark:text-white">
        ...
        <button id="new-match-btn" class="w-full py-4 bg-gray-900 dark:bg-white text-white dark:text-[#0f172a] font-black rounded-2xl transition hover:opacity-90 active:scale-[0.98] uppercase tracking-widest">${__tsT('tennis.newMatch','Main Lagi')}</button>
      </div>
    </div>
